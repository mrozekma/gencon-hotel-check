...
* `--key` is the only mandatory argument, specifying either your individual Passkey ID number (if you haven't booked a room yet) or your reservation acknowledgement number (if you have). You can find this via the [Gencon Housing](https://www.gencon.com/housing) page.
  * If you haven't booked a room, click the "Go to Housing Portal" button and you will end up on a page with a URL of the form `https://aws.passkey.com/reg/XXXXXXXX-XXXX/null/null/1/0/null`. Pass `XXXXXXXX-XXXX` as the key argument to the script.
  * If you have booked a room, your acknowledgement number is on the Housing page, in parentheses in the status column. Pass this as the key argument to the script, and also pass `--surname NAME` where `NAME` is the last name of one of the guests on the reservation.
